...
const Appointments: React.FC = () => {
  ...
  const generateBill = async (customer: Customer, appointmentsToBill: Appointment[], mode: string) => {
    ...
    items.forEach((item) => {
        ...
        doc.text(`₹${item.price.toFixed(2)}`, pageWidth - 25, yOffset, { align: 'right' });
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(12);
    doc.text(`Total: ₹${totalAmount.toFixed(2)}`, pageWidth - 25, yOffset, { align: 'right' });
                                                    ...
                                                    <div key={appt.id} className="flex flex-col sm:flex-row sm:items-center justify-between text-sm gap-2">
                                                        ...
                                                        <div className="flex items-center gap-4">
                                                             ...
                                                             <select 
                                                                ...
                                                                className={`text-xs font-medium rounded-full px-2 py-1 border border-transparent focus:outline-none focus:ring-2 focus:ring-offset-1 ${
                                                                    appt.status === AppointmentStatus.Scheduled ? 'bg-blue-100 text-blue-800 focus:ring-blue-500' :
