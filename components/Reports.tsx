...
const Reports: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'sales' | 'customers' | 'inventory' | 'services' | 'attendance'>('sales');
    ...
    const getAttendanceReports = () => {
        ...
        return filtered.map(a => {
            ...
            if (logout) {
                ...
                overtime = diffHours > 9 ? Math.floor(diffHours - 9) : 0;
            }
    ...
    const attendanceReport = getAttendanceReports();
    ...
    const handleExport = () => {
        ...
        else if (activeTab === 'attendance') exportToCSV(attendanceReport, 'attendance_report');
    };
        ...
        <div className="space-y-6">
            ...
            <div className="border-b border-gray-200">
                <nav className="-mb-px flex space-x-8">
                    {['sales', 'customers', 'inventory', 'services', 'attendance'].map((tab) => (
                        <button
                            key={tab}
            ...
            {activeTab === 'attendance' && (
                <div className="space-y-6">
                    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                        <h3 className="text-lg font-medium text-gray-900 mb-4">Staff Attendance & Overtime Log</h3>
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">Logout</th>
                                        <th className="px-6 py-3 text-center text-xs font-bold text-gray-500 uppercase">Total Hours</th>
                                        <th className="px-6 py-3 text-center text-xs font-bold text-gray-800 uppercase">Overtime (Hr)</th>
                                    </tr>
                                </thead>
                                <tbody className="bg-white divide-y divide-gray-100">
                                    {attendanceReport.map((entry: any) => (
                                        <tr key={entry.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                {entry.logoutTime ? new Date(entry.logoutTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--:--'}
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-600">
                                                {entry.totalHours}h
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-center">
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold ${entry.overtime > 0 ? 'bg-amber-100 text-amber-800 border border-amber-200' : 'bg-gray-100 text-gray-400'
                                                    }`}>
                                                    {entry.overtime > 0 ? `+${entry.overtime} hr` : '0 hr'}
                                                </span>
                                            </td>
                                        </tr>
                                    ))}
