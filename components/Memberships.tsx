...
import { Package, GenderTarget } from '../types';
import { Crown, Check, Plus, Edit2, Trash2, Download, Wallet, Calendar, Gift } from 'lucide-react';
import Modal from './ui/Modal';

const Memberships: React.FC = () => {
    const [tiers, setTiers] = useState<Package[]>([]);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingTier, setEditingTier] = useState<Package | null>(null);
    ...
    useEffect(() => {
        setTiers(db.packages.getAll());
    }, []);

    const openModal = (tier?: Package) => {
        if (tier) {
            setEditingTier(tier);
            setFormData({
                name: tier.name,
            ...
            setEditingTier(null);
            setFormData({ name: '', cost: 0, creditValue: 0, description: '', validityMonths: 12, complimentaryServices: '', gender: 'Unisex' });
        }
        setIsModalOpen(true);
    };
        ...
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-2">
                    <button
                        onClick={() => exportToCSV(tiers, 'packages')}
                            ...
                            <div className="p-8 flex-1 flex flex-col">
                                <div className="flex justify-between items-center mb-4">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-[10px] items-center uppercase tracking-wider font-bold text-gray-400">
                                            {tier.gender}
                                        </span>
                                    </div>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingTier ? "Edit Package" : "Add Membership Package"}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Gender Target</label>
                            <select
